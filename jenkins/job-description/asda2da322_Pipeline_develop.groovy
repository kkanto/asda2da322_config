...
def job_config = [
        job: [
                name: "asda2da322_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'asda2da322_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/asda2da322_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.asda2da322.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "asda2da322", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("asda2da322_Pipeline_develop");

job.with {

    authenticationToken('8v6NxUKFmLOMMScuGMjchPk7MvAYoHEB')
